...
# Lazy-loaded HF NER pipeline for fallback entity extraction
_NER_PIPELINE = None
_NER_PIPELINE_ATTEMPTED = False

def _get_ner_pipeline():
    global _NER_PIPELINE, _NER_PIPELINE_ATTEMPTED
    if not _NER_PIPELINE_ATTEMPTED:
        _NER_PIPELINE_ATTEMPTED = True
        try:
            from transformers import pipeline
            # Force CPU so it doesn't compete with the main model for VRAM.
            # Use aggregation_strategy (replaces deprecated grouped_entities).
            ...
            print(f"\n[WARNING] Failed to load fallback NER pipeline: {e}")
            _NER_PIPELINE = None
...
def compute_ner(data):
    ...
    for entry in data:
        ...
        # If model output cannot be parsed into entities, fallback to a token-classification NER
        # applied on the original input text (if available).
        if (not pred or len(pred) == 0) and entry.get('input'):
            try:
                ner_pipe = _get_ner_pipeline()
                if ner_pipe is not None:
                    ner_res = ner_pipe(entry.get('input'))
                    pred = []
                    from nereval import Entity
                    for it in ner_res:
                        word = it.get('word') or it.get('entity') or ''
                        label = it.get('entity_group') or it.get('entity') or ''
                        if word and label:
                            # normalize
                            w = word.strip()
                            l = label.strip()
                            pred.append(Entity(w, l, 0))
            except Exception:
                pass
